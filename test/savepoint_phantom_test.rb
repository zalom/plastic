...
# Tests for Bridge.savepoint_phantom_lines (intent 134): a pure, disk-only detector for a
...
class SavepointPhantomTest < Minitest::Test
  ...
  def reasons
    Bridge.savepoint_phantom_lines(@dir).map(&:last)
  end

  def lines
    Bridge.savepoint_phantom_lines(@dir).map(&:first)
  end
  ...
  def test_absent_ledger_returns_empty
    ...
    assert_equal [], Bridge.savepoint_phantom_lines(@dir)
  end

  def test_clean_ledger_built_by_the_real_writer_returns_empty
    write("spec.md")
    Bridge.append_savepoint(@dir, File.join(@dir, "spec.md"), now: Time.utc(2026, 1, 1))
    assert_equal [], Bridge.savepoint_phantom_lines(@dir)
  end
  ...
  def test_flags_file_landing_line_when_file_is_still_a_sentinel_placeholder
    write("spec.md", "#{Bridge::PLACEHOLDER_SENTINEL}\n\nplaceholder\n")
  ...
  def test_returns_every_phantom_line_in_a_mixed_ledger
    ...
    result = Bridge.savepoint_phantom_lines(@dir)
