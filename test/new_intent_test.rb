...
class NewIntentTest < Minitest::Test
  ...
  # Intent 390: Plastic runs no version control command, and its tests never build or
  # inspect a real git repository. Git itself drops an empty directory unless it holds
  # a tracked file, so `.gitkeep` existing on disk right after scaffolding is the whole
  # proof: any checkout that includes the actions/ directory keeps it non-empty.

  def test_actions_dir_has_a_gitkeep_so_it_survives_any_checkout
