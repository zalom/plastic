# ACTION_3: Packaging guard proving the founding no-store-data-ships rule

New file: `test/packaging_no_store_ids_test.rb`. Independent of ACTION_1/2's exact commit
boundary but most meaningful once ACTION_2 has actually removed the amnesty file (its own
green assertion is the proof case for that removal).
...
- `ruby test/packaging_no_store_ids_test.rb` passes both assertions once ACTION_2 has removed
  the amnesty file.
- Manually confirmed (during Why, not re-derived at Exec) that running `violations_in` against
  every `.freeze`d array/hash literal currently in `scripts/` (25+ candidates: `doctor.rb`'s
  `REQUIRED_SCRIPTS`, `CLAUDE_HOOK_EVENTS`; `hook_registry.rb`'s `CODEX_PRE_HOOKS` etc.;
  `roadmap_queue.rb`'s `STATUSES`; `lock.rb`'s `TYPES`; and 20 more) finds zero matches at the
  5-token threshold - every one uses letter-leading tokens. No exemption list is needed; if this
  changes in the future (a legitimate shipped literal happens to cross the threshold), the fix
  is to narrow `FOLGEZETTEL_TOKEN` or raise the threshold, never to hand-exempt a filename.
