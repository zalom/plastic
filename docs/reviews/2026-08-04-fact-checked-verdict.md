...
| # | Issue | What happens today | Fix | What you get |
|---|---|---|---|---|
| 15 | "Codify the loop" already has an intent | Intent 213 holds the advisor verdict of 2026-07-18: convert hot procedures to scripts, one at a time, no new concepts; three decisions wait for the owner. It is a Future intent, never started | Activate 213 and run the normal cycle; merge the script list (next-step, boarding in one call, scoped store commits) into it | The work lands inside the intent that already decided how to do it |
