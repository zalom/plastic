# Guide

This guide is for a person who uses Plastic. To change Plastic, read
[CONTRIBUTING.md](../../CONTRIBUTING.md).

## Get started

| Page | What it answers |
| ---- | --------------- |
| [Installation](getting-started/installation.md) | How do I install it? |
| [Quick start](getting-started/quick-start.md) | What do I run first? |
| [Intent commands](getting-started/intent-commands.md) | How do I carry one piece of work from its first line to its close? |
| [Project and roadmap commands](getting-started/project-and-roadmap-commands.md) | How do I register a project and read a roadmap? |
| [Auto and session commands](getting-started/auto-and-session-commands.md) | How does an auto team take an intent, and how does a session keep its ledger? |
| [Search commands](getting-started/search-commands.md) | How do I find text in the stores? |
| [Ask a question](getting-started/ask-a-question.md) | How do I search with a whole question? |
| [Backup command](getting-started/backup-command.md) | How do I back up and restore everything? |
| [Migrate command](getting-started/migrate-command.md) | How do I move the stores under `stores/`? |
| [Configuration](getting-started/configuration.md) | What can I set, and where? |
| [What Plastic covers](getting-started/what-plastic-covers.md) | Which commands exist? |
| [Supported agents](getting-started/supported-agents.md) | Which coding agents does it install into? |
| [Troubleshooting](getting-started/troubleshooting.md) | What does an exit code or an error mean? |

## Task guides

The task guides from before the command line stay in [docs/guides/](../guides/index.md). They
were written for the 1.x skills. The skills no longer ship; where a guide names a skill, use the
`plastic` command that the guide index names instead.
