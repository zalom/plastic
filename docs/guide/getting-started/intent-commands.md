...
| `plastic intent step ID` | Runs the next ready step of the intent's graph. |
| `plastic intent answer ID --node NODE --decision "TEXT"` | Answers a step that waits for a decision. |
| `plastic intent verify ID` | Runs the checks that an intent must pass before it closes. |
