...
A booted session rests in direct mode, where the prompt itself is the action and the work runs inline rather than through a dispatched agent. The `plastic-direct` skill routes each prompt in a single read, on a time estimate made from the prompt alone: a bounded change of about five minutes or less runs now, a prompt that one answer would settle gets one clarifying question and then runs, and anything that stays vague or exceeds the budget is offered a thinking intent (`plastic-intent-speccing`) rather than guessed at. An explicit "auto" hands off to `plastic-auto`. The skill itself ships no wiring: intent 298 owns the capture hook, the session-start mode line, and the per-session day-ledger pointer, and intent 305 owns the `PLASTIC.md` core block that states every session starts in direct mode. Until both land, `plastic-direct` is installed and idle.
