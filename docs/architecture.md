...
Every agent in `agents/*.md` pins an explicit Claude Code model alias (`opus` or `sonnet`) in its frontmatter: never `inherit`, never Fable by default, unless an explicit `agents.models.<name>` config override names Fable for that role, in which case the override is honored as written. Consultation agents (plastic-fable-advisor) are the shipped exception: they pin fable in frontmatter by design, are never dispatched by the auto pipeline, and downgrade via agents.models overrides like any other agent. The tier is by role weight, not by stage:
...
### consultation agents (intent 185)

`plastic-fable-advisor` is a Claude-only role file pinned to `model: fable` in shipped frontmatter, tracked separately from the lifecycle tiers by `AgentModels::CONSULTATION_AGENTS` (excluded from `TIER_DEFAULTS`, so it carries no built-in Codex reasoning-effort mapping). It ships one agent at effort medium; the S/M/L tier travels in the caller's brief (a TIER line), not as separate agent files, so a per-call effort raise to xhigh for L consultations is the caller's responsibility where the harness supports it. `InstallerCore#generate_codex_agents` reads each agent's AUTHORED frontmatter model before overrides apply and skips any file pinned to `fable`, since Codex has no fable alias; an `agents.models.<name>` override that flips a non-fable agent's authored model to `fable` is still honored as written (intent 170), and an override on the consultation agent itself still rewrites its frontmatter through the normal `install_agents` path.

The Operating Manual injection is model-conditional and fail-open, implemented in `scripts/lib/opus_manual.rb` (`OpusManual`, pure logic, dependency-injected). The primary path is `scripts/hook-session-start`: it reads the optional `model` field off the SessionStart hook's stdin JSON and, when it matches Opus, appends the manual text plus an advisor pointer to `additionalContext`, gated by a `context.opus_manual` config check and a per-session marker file that prevents a double injection. The fallback path covers the field's absence (a `/clear` start, or a mid-session `/model` switch): `hooks/statusline` caches the last-seen `display_name` per session id to `/tmp/plastic-model-<sid>` (bash 3.2, no ruby, no jq), and a new `UserPromptSubmit` hook (`hooks/opus-manual` calling `scripts/hook-opus-manual`, registered in `HookRegistry.events` so both the flat-install `settings.json` merge and the plugin `hooks/hooks.json` wire it) reads that cache and injects once when the primary path did not fire, sharing the same marker file so the two paths never double-inject. `OpusManual.manual_text` resolves `plugin_root/manuals` first, falling back to a DI'd directory (default `~/.plastic/manuals`) when the plugin root is empty or the file is unreadable; `InstallerCore#core_files` syncs both manuals into `~/.plastic/manuals` on every install and update, so the fallback resolves even under a flat, non-plugin install with an empty `CLAUDE_PLUGIN_ROOT`.
