...
The session bridge is a small JSON file under `/tmp` that records the active intent and whether auto mode is armed; the arm, disarm, and repair paths and the spawn preamble read it. Resolving which bridge to use has a fixed precedence (intent 52). Claude Code does not export a session id env var into the hook environment; it passes `session_id` on the hook stdin JSON, so the hooks parse it out of stdin (in Ruby, never in bash). The resolver picks the first non-empty of: the stdin `session_id`, the `CLAUDE_CODE_SESSION_ID` environment variable (the headless/background id), and a derived `auto-<digest>` key (a short hash of the store path plus the intent id). The derived key is deterministic, so a session-less arm and a later session-less check land on the same bridge file rather than writing a null-session bridge. A null session can never be persisted. Bridge discovery is strictly per-session (intent 90): a foreign session's bridge is never returned, and a session that owns no bridge resolves to nothing.
...
The skill-authoring guides (`skill-creating`, `skill-evaluating`) left the installed skill tree in 2.0 (intent 304) and live under `docs/skill-authoring/` as reference material.
