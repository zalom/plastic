...
Controller, delegate, and claim records answer different questions. The controller owns the intent, an active delegate is a separately authorized child session, and a claim identifies the current writer for one artifact. Active delegates remain in the authorization set. Finished and failed delegates remain as descriptive history, bounded to the 20 most recent terminal entries.
