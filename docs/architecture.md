...
Two consultation role files are tracked separately from lifecycle tiers by `AgentModels::CONSULTATION_AGENTS`, and neither is dispatched by the auto pipeline. On Claude Code, `plastic-advisor` ships with `model: fable` and `plastic-faux-advisor` with `model: opus`. On Codex, the installer generates OpenAI equivalents: `gpt-5.6-sol` and `gpt-5.6-terra`. The faux advisor carries the full Operating Manual in its body. Model overrides use the same harness-scoped `agents.models.<harness>.<name>` mechanism as every other agent.

The `plastic-agent-advisor` skill (`skills/agent-advisor/`) is the one front door. It teaches when consulting is worth the money and reads the harness-scoped config to route to the configured agent. Every advisor and lifecycle agent ships at medium effort on Claude Code and Codex. `agents.efforts.<harness>.<name>` is the explicit override, with project config over global.
...
Model and effort are shipped defaults, independently overridable through `agents.models.codex.<name>` and `agents.efforts.codex.<name>`. A literal Codex model ID still receives medium effort unless effort is explicitly overridden. Advisor TOMLs use `gpt-5.6-sol` and `gpt-5.6-terra`, both at medium.
...
The `plastic-intent-continuing` skill is the front door that then continues work: it is a thin router, deciding among three routes and dispatching to exactly one. The project route (`plastic-intent-continuing`) is the default for a bare "continue": it lands on the board (the project board when a project is loaded, otherwise the global board, invoking the renderer rather than rendering itself), presents choices, and stops. The intent route (`plastic-intent-continuing`) fires when a specific intent is named to resume: it reads that intent's `savepoint.md` ledger FIRST (intent 81) and hands off to `plastic-intent-continuing`, which takes the lock and runs the cycle from there. The roadmap route (`plastic-intent-continuing`) resumes a mid-flight roadmap by calling `scripts/roadmap-next` (intent 148), which liveness-ranks the tier's roadmap files and selects the frontier batch deterministically, rather than ranking by eye. None of the three routes run the health check, load core, or set the statusline, all of which the hooks already did, and none drive work autonomously on their own (that is `plastic-auto`, reached only through `plastic-intent-continuing`'s or `plastic-intent-continuing`'s auto branch). On the intent route, the last ledger line classifies the state (a cycle position, or `Done delivered|abandoned` for a terminal intent), and it verifies only that last line's artifact before continuing, rebuilding the ledger from files on drift. The ledger's fixed bookends, a born `What created` first line and a `Done` last line, make state a single read instead of a filesystem probe.
...
The End tail runs in a fixed order, and the QMD reindex is always last: `outcome.md`, then the INDEX terminal move, then the savepoint `Done` line, then the commit, then disarm (worktree release, then `Lock.release`), and finally the QMD reindex. Running the reindex after disarm keeps the search index from referencing a lock that has just been released. The post-done access window is bounded by the delivery lock, `[INDEX terminal to Lock.release]`: while the lock is held the completing session keeps full access, and once the lock is released the directory is frozen (writable again only on an explicit owner grant; there is no maintenance lock, the one intent 112 proposed was abandoned).
