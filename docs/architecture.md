...
The delivery lock (`delivery.lock` in the intent directory) names the session delivering an intent, as owner or delegate; `scripts/lib/arm.rb` behind `plastic auto start` (which runs `plastic-lock arm`) is how a team takes and gives back an intent. Given a roadmap slug, `plastic auto start` arms the roadmap's first ready intent instead (intent 391). A lock held by another session is refused with exit 3 and a public hint, never taken over.
...
`auto start --allow-inline` carries that approval to the existing lock guard.
