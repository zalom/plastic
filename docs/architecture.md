...
3. **Boot banner and version**: the hook prepends `Plastic Core loaded — v{version}` to the session context on a clean boot, or `Plastic Core loaded with issues — {check}: {message} — run /plastic-doctor` when a core check fails. The same line is also emitted on the top-level `systemMessage` channel so it is visible in the user's terminal (the session context is model-facing only). One `BootBanner` renderer feeds both channels, so they cannot drift (intent 54). The hook never blocks (always exits 0).
