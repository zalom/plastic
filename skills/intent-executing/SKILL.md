...
Dispatches subagents to do the work. The controller never implements. It dispatches, reviews, and tracks progress. How many dispatches it makes depends on the tier stamped in `plan.md` (the `Tier:` line, copied from `spec.md`):
...
Read its response with the same codes as (b) below. Run no per-task spec review and no per-task quality review at these tiers. Step 3's final review covers the work.
...
### Step 3: Final Review
After all tasks complete, dispatch a final reviewer for the entire implementation. This runs at every tier, S, M, and L. The reviewer is a separate agent with fresh context and is never the maker.
