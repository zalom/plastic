...
5. **Print the report screen, then continue at that stage.** For "where are we" on one named
   intent, run `ruby ~/.plastic/scripts/report-screen state <intent_dir>` and print its output
   as it is: the title, the field table, the `Changed` row, and the Steps table come from the
