    ...
    module Commands
      class IntentStep < IntentCommand
        ...
        def graph_owner?
          ...
          raise Refusal, "return not accepted: take intent #{id} with plastic auto start #{id} --project #{scope.slug} before submitting it" unless Array(options[:returns]).empty?

          @output.next_step("plastic auto start #{id}", because: "graph execution requires this session to take the delivery lock")
          false
        end
