...
module NodeLedger
  ...
  #
  # Two line shapes are read (one line per node, inside `## Graph` only), so
  # one authored graph.md parses under this seam and under 334's real parser
  # both, and the dogfood file does not become unreadable when the branches
  # meet:
  #   - <node-id> needs <space-separated node ids, or "nothing">   (334)
  ...
  # A root is spelled "none" here and "nothing" by 334; both mean no targets.
  GRAPH_ROOT_TARGETS = %w[none nothing].freeze
  private_constant :GRAPH_ROOT_TARGETS
  ...
  def needs_from_graph(graph_path, node)
    ...
    section.each_line do |line|
      ...
      targets = rest.split(/[,\s]+/).map(&:strip).reject(&:empty?)
      return [] if targets.length == 1 && GRAPH_ROOT_TARGETS.include?(targets.first.downcase)
