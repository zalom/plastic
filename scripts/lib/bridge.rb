...
module Bridge
  ...
  # unchanged.
  #
  # Worktree-membership-first carve-out (intent 168): hook-code-gate passes
  # the edited file as edited_path; when that file lies inside a provisioned
  # code worktree (<repo>/.claude/worktrees/{id}--{slug}), only that
  # worktree's owning intent may gate the write, BEFORE active_for's
  # per-session filter (intent 90). The owner is the session row whose cwd
  # (worktree.code in to_bridge_data) IS that dir, newest last_seen_at on a
  # tie; no owner resolves nil, so a session-keyed guided row can never claim
  # a write located inside a sibling intent's worktree. No edited_path (every
  ...
  def self.discover_bridge(session:, cwd: Dir.pwd, store_home: nil, edited_path: nil)
    home = store_home || resolve_store_home(cwd: cwd)
    conn = Plastic::DB.connect(home)
    return nil if conn.nil?

    wt_dir = blank?(edited_path) ? nil : enclosing_worktree_dir(edited_path)
    row =
      if wt_dir
        owners = Plastic::DB::Sessions.all(conn).select do |r|
          !blank?(r["cwd"]) && File.expand_path(r["cwd"]) == wt_dir
        end
        owners.max_by { |r| r["last_seen_at"].to_s }
      else
        Plastic::DB::Sessions.active_for(conn, session: session, cwd: cwd)
      end
  ...
  # (ACTION_5/11). Per-intent savepoint.md nearly retires from the LIVE flow:
  # the ONE surviving live write is the terminal Done bookend line
  # (append_terminal_savepoint, intent 161 contract); every other milestone
  # stamps DB rows only. Existing savepoint.md files are left in place
  ...
  SAVEPOINT_FILE = "savepoint.md" # live write: terminal Done bookend only; otherwise a legacy read target (doctor, immutable history)
  ...
  def self.stamp_savepoint_event(intent_dir, stage, event_type, session: nil, now: Time.now, payload: {}, commit: true)
    store_home, intent_id = store_and_id_for(intent_dir)
    ...
    export_and_commit_savepoint(intent_dir, stage: stage, event_type: event_type, commit: commit) if stamped
    stamped
  ...
  #
  # The bookend lands TWICE by design (intent 161's contract, kept across the
  # intent-41 cutover): the `Done` line still lands in the per-intent
  # savepoint.md file, the fixed last-line bookend end-intent and
  # plastic-intent-continuing classify state from in a single read, AND the
  # savepoint_events row below mirrors it as the authoritative DB ledger
  # event. The file write is the one savepoint.md live write that survives
  # the cutover; every other milestone stays DB-only.
  def self.append_terminal_savepoint(intent_dir, disposition, session: nil, now: Time.now, commit: true)
    ...
    unless existing.each_line.any? { |l| l.split(/\s+/)[1] == "Done" }
      File.open(path, "a") { |io| io.write("#{now.utc.iso8601}  Done  #{disposition}\n") }
    end

    stamp_savepoint_event(intent_dir, "Done", disposition, session: session, now: now, commit: commit)
  end
  ...
  def self.export_and_commit_savepoint(intent_dir, stage:, event_type:, commit: true)
    ...
    commit_savepoint_export(store_home, path) if commit
    true
  rescue StandardError => e
  ...
  def self.arm(session, intent_id:, intent_dir:, store:, name:, auto:)
    ...
    when :held
      raise LockHeldError, "delivery lock for intent #{intent_id} is held by " \
        "session #{lease_row && lease_row['owner_session']}; run /plastic-doctor " \
        "check the lock status"
    when :stale
      raise LockHeldError, "delivery lock for intent #{intent_id} is stale " \
        "(owner #{lease_row && lease_row['owner_session']}); run /plastic-doctor " \
