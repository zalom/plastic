...
# Installer engine, instantiable with injected package root / store / agent map so
# tests can run it hermetically (no eval, no global-constant rewriting). Mirrors the
# DI recipe proven in doctor.rb (intent 30a).
class Installer
  ...
  def cli(argv = ARGV)
    ...
    selected = flags[:agents].empty? ? prompt_agents : flags[:agents]
    ...
    puts "Mode: #{mode}"
    puts "Agents: #{selected.map { |k| agent_config(k)[:name] }.join(", ")}\n\n"

    distribute(mode)
    bootstrap if mode == :install

    results = selected.map { |key| install_for_agent(key, flags[:force]) }
...
if $PROGRAM_NAME == __FILE__
  Installer.new(package_root: ENV["PLASTIC_PACKAGE_ROOT"] || File.expand_path("..", __dir__)).cli(ARGV)
end
